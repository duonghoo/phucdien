			  ...
			  <div class="card col-12 col-md-6 col-lg-3 mb-5 mt-3" style="max-width: 50rem">
                <div class="mx-1 content d-sm-block w-100">
                  ...
                  <div class="card-body flex-column justify-content-center">
					<a href="{{getUrlPost($item)}}"><h5 class="card-title text-center fs-16" style="margin-top:1rem; margin-bottom:1rem">{{$item->title ?? ""}}</h5></a>
					<div class="mx-2 mb-1">
					...
					@else
					<button class="btn-product text-center add-cart w-100" id="addcart{{$item->id}}" value="{{$item->id ?? ""}}">{{__('mes.remove')}}</button>
					@endif
					@else
					<button class="btn-product text-center add-cart w-100" id="addcart{{$item->id}}" value="{{$item->id ?? ""}}">{{__('mes.add')}}</button>
					@endif
